# -*- coding: utf-8 -*-
"""
Created on Tue Feb 18 12:51:22 2025
...
temperatura = st.selectbox("Selecciona Temperatura (°C)", [36.5, 37.2, 36.8, 37.5, 36.9, 37.0, 35.3, 37.1, 34.9, 38.1, 36.7, 37.22], key="temp_original")
presion = st.selectbox("Selecciona Presión Arterial (mmHg)", [120, 125, 130, 140, 150, 160], key="presion_original")
comorbilidad = st.selectbox("Selecciona Comorbilidad", label_encoder_comorbilidad.classes_, key="comorbilidad_original")
...
# Entrada del usuario para Predicción con Ruido (en la presión arterial)
st.subheader("Ingresar Datos para Predicción con Ruido en la Presión Arterial")
temperatura_irrelevante = st.selectbox("Selecciona Temperatura (°C) para Ruido", [36.5, 37.2, 36.8, 37.5, 36.9, 37.0, 35.3, 37.1, 34.9, 38.1, 36.7, 37.22], key="temp_ruido")

# Introducimos un ruido aleatorio en la presión arterial (solo valores mayores a 180)
presion_irrelevante = st.selectbox("Selecciona Presión Arterial (mmHg) para Ruido", [190, 200, 210, 220, 230, 240], key="presion_ruido")

# Comorbilidad original
comorbilidad_irrelevante = st.selectbox("Selecciona Comorbilidad", label_encoder_comorbilidad.classes_, key="comorbilidad_ruido")

# Predicción con presión arterial ruidosa
...
st.write(f"### Resultados con Ruido en la Presión Arterial:")
st.write(f"Presión Arterial Introducida: **{presion_irrelevante} mmHg**")
st.write(f"Clase Predicha con Ruido: **{clase_predicha_con_ruido}**")
